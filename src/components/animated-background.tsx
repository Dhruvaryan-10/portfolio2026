...
const AnimatedBackground = () => {
  ...
  // Use the shared context so SkillInfoCard can render outside the canvas
    ...
    const start = () => {
      ...
      Object.values(SKILLS)
        ...
        .forEach((skill, idx) => {
          ...
          if (!keycap) return;
          // Priority skills bounce higher — they rise above the rest visually
          const maxY = skill.isPriority
            ? Math.random() * 250 + 350
            : Math.random() * 200 + 200;
          const t = gsap.to(keycap.position, {
            y: maxY,
            duration: skill.isPriority
              ? Math.random() * 1.5 + 1.5
              : Math.random() * 2 + 2,
            delay: idx * 0.6,
            repeat: -1,
